...
function Modal() {
  ...
  const uploadPost = async () => {
    ...
    //4. get download url from fb storage and
    await uploadString(imageRef, selectedFile, "data_url").then(
      async (snapshot) => {
        const downloadUrl = await getDownloadURL(imageRef);

        //5. update the original post with image
  ...
  const addImageToPost = (e) => {
    ...
    reader.onload = (readerEvent) => {
      //when browser read file
      setSelectedFile(readerEvent.target.result); //get a file as as object and store to state
    };
  };
    ...
    <Transition.Root show={open} as={Fragment}>
      ...
      sm:p-0"
        >
          ...
          {/*kthis element is to trick the browser into centering the modal contents */}
          <span
